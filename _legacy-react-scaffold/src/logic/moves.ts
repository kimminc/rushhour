...
import { occupancySet } from './board'
 ...
 */
export function maxSlide(state: GameState, vehicleId: string, dir: Axis1): number {
...
export function canMove(state: GameState, vehicleId: string, dir: Axis1, steps: number): boolean {
  if (steps < 1) return false
  return steps <= maxSlide(state, vehicleId, dir)
}

/** 이동이 불가능하면 원래 상태를 그대로 반환한다 (오류를 던지지 않음 — 호출부에서 canMove로 먼저 확인). */
export function applyMove(state: GameState, vehicleId: string, dir: Axis1, steps: number): GameState {
 ...
 */
export function serializeMove(vehicleId: string, dir: Axis1, steps: number): string {
  return `${vehicleId}${dir > 0 ? '+' : '-'}${steps}`
}

const MOVE_TOKEN_RE = /^(.+)([+-])(\d+)$/

export function parseMove(token: string): { vehicleId: string; dir: Axis1; steps: number } {
...
export function replaySolution(state: GameState, tokens: string[]): GameState {
