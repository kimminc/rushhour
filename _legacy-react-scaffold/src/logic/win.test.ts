...
describe('isSolved (엄격판 — 목표 차량이 경계 마지막 칸에 도달)', () => {
  it('오른쪽 출구: 전방이 마지막 열에 닿아야 승리', () => {
    ...
    expect(isSolved(s)).toBe(false) // front = col+length-1 = 4, cols-1=5
    const moved = applyMove(s, 'X', 1, 1)
    expect(isSolved(moved)).toBe(true) // front = 5
  })

  it('로드 즉시 이미 풀린 상태는 감지되어야 한다 (validatePlacement와는 별개로 isSolved 자체 검증)', () => {
    const s: GameState = {
  ...
  it('방향/정렬이 다르면 승리로 판정하지 않는다', () => {
    ...
    expect(isSolved(s)).toBe(false) // row(1) !== exitRow(2)
  })
  ...
  it('세로 출구(bottom)도 동일 규칙으로 동작한다', () => {
    ...
    expect(isSolved(s)).toBe(false) // front = row+length-1 = 4, rows-1=5
    const moved = applyMove(s, 'X', 1, 1)
